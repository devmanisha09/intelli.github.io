...
//        switch services and do operation
        switch ($feedData['list_id']){
            case 'single-sms':
                ...
                $this->handleSMSResponse($action, $smsData, $feed, $entry, $form);
                break;
            case 'sms-campaign':
                if (empty($feedData['message_body']) ||
            ...
            case 'create-new-contact':
                ...
                if (is_wp_error($response)) {
                    $this->handleFailed($form, $entry, $feed, $response->get_error_message());
                } else {
                    $this->handleSuccess($form, $entry, $feed,'ClickSend SMS feed has been successfully initialed and add subscriber in contact list' );
                }
                break;
            case 'add-contact-list':
                ...
                if (is_wp_error($response)) {
                    ...
                    $this->handleSuccess($form, $entry, $feed,'ClickSend SMS feed has been successfully initialed and add contact list' );
                }
            break;
            case 'email-campaign':
                if (
                ...
                if (is_wp_error($response)) {
                    ...
                    $this->handleSuccess($form, $entry, $feed,'ClickSend SMS feed has been successfully initialed and pushed email campaign data' );
                }
                break;
        }
    }
    ...
    public function handleSMSResponse($action, $smsData, $feed, $entry, $form)
    {
        ...
        if (is_wp_error($response)) {
            ...
            $this->handleSuccess($form, $entry, $feed );
        }
    }

    public function handleFailed($form, $entry, $feed, $err_msg = 'ClickSend Integration Data insert failed.')
    {
    ...
    public function handleSuccess($form, $entry, $feed, $success_msg = 'ClickSend feed has been successfully initialed and pushed data')
    {
        // It's success
        do_action('ff_log_data', [
            'parent_source_id' => $form->id,
